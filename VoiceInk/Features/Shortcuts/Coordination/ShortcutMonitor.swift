...
final class ShortcutMonitor {
    ...
    private func handleModifierOnlyShortcut(
        ...
        let disposition = Self.modifierOnlyEventDisposition(
        ...
        guard disposition != .ignore else { return false }

        if state.isDown {
            if case .keyUp(let shouldSuppress) = disposition {
                let shouldTrigger =
